...
export const QuadrantEditor: React.FunctionComponent = () => {
  ...
  useEffect(() => {
    if (routeQuadId > 0) {
      callDataApi((baseUrl) =>
        api.QuadrantApiFactory(undefined, baseUrl).quadrantIdGet(routeQuadId),
      ).then((result) => {
  ...
  const handleSubmitEditForm = (e: MouseEvent<HTMLButtonElement>): void => {
    ...
    if (routeQuadId) {
      promise = callDataApi((baseUrl) =>
        api
          .QuadrantApiFactory(undefined, baseUrl)
          .quadrantIdPut(routeQuadId, quadrant),
      );
    } else {
      promise = callDataApi((baseUrl) =>
        api.QuadrantApiFactory(undefined, baseUrl).quadrantPost(quadrant),
      );
    }
    promise
      .then(() => {
    ...
    <form className="needs-validation" noValidate>
      ...
      <div className="form-group">
        ...
        <input
          ...
          value={quadrant.name}
          onChange={(e) =>
            setLocalQuadrant({ ...quadrant, name: e.target.value })
          }
        />
      ...
      <div className="form-group">
        ...
        <ColorPicker
          ...
          onColorChange={(e: string) =>
            setLocalQuadrant({ ...quadrant, color: e })
          }
        />
